...
1. Promote foundational visual decisions into `@fresh/tokens`, `@fresh/ui-core`, and
   `@fresh/ui` only after the board is approved.
2. Promote a product-derived pattern into `@fresh/ui` only if it is repeated across
   products, clearly canonical, or required for the design-system contract.
3. Keep product-specific composites out of the shared package until they are proven
   reusable.
4. If a pattern is mainly layout scaffolding, consider `@fresh/ui-core` or app-level
...
| --- | --- | --- | --- | --- |
| `PageHeader` | Drugbook, Form Builder | `@fresh/ui` | Repeats across product surfaces and will stabilize page framing for AI generation | Should support title, description, metadata, and actions |
| `SectionHeader` | Drugbook, Form Builder | `@fresh/ui` | Common section framing pattern for cards, forms, and grouped lists | Keep compact and composable |
| `StickyActionFooter` | Drugbook | `@fresh/ui` after a second use case, otherwise app-level | Useful but needs another product example before becoming canonical | Likely mobile-first with responsive desktop behavior |
| `EmptyState` | Form Builder | `@fresh/ui` | Very common shared pattern and helpful for agent determinism | Should support icon, title, body, and actions |
| `PreviewPanel` or `BrowserPreviewShell` | Form Builder | app-level first, later `@fresh/ui` if reused | Helpful pattern, but probably too product-shaped today | Keep out of shared layer for now |
...
| Candidate | Source boards | Recommended home | Why it matters | Notes |
| --- | --- | --- | --- | --- |
| `IconButton` | Drugbook, Form Builder | `@fresh/ui` | Repeated utility action pattern that should not be improvised in screens | Needs strict size, focus, and accessibility rules |
...
3. Implement `IconButton` and `EmptyState` first because they unlock many screens with
   low architecture risk.
4. Implement `PageHeader` and `SectionHeader` next to stabilize page composition.
5. Keep `StickyActionFooter`, `FilterBar`, and `SummaryTotals` as recipes until they
   prove reusable across more than one product.
